...
(function(){
  var VER = '2026-07-12b';
  if (window.__budActive === VER) return;   // deze versie draait al
  window.__budActive = VER;
  window.__budWidgetLoaded = true;          // laat oudere Bud-versies (die hierop letten) stoppen
  ...
  function boot(){
    var olds = document.querySelectorAll('#bud'); for (var i=0;i<olds.length;i++){ olds[i].remove(); }  // verdring een oudere widget
...
(function(){
  ...
  var BUD_CONFIG = {
    ...
    formspree: "",
    // Geeft een Promise<boolean> terug: true = aangekomen bij Formspree, false = mislukt.
    submit: function(payload){
      ...
      try{
        ...
        if(now - last < 20000 || count >= 5) return Promise.resolve(true);   // rate limit: bots & dubbelkliks
      ...
      return fetch(endpoint, {
        ...
        if(window.console) console.error("[Bud] netwerkfout bij verzenden:", err);
        return false;
      });
    }
  };
